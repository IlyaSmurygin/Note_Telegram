...
- **Заметки** уникальны → создаем каждый раз
...
```
    ...
    ▼             ▼          ▼
┌────────┐   ┌────────┐  ┌──────────┐
│  Note  │   │  Task  │  │ Reminder │  ← CREATE (каждый раз новый)
└───┬────┘   └───┬────┘  └────┬─────┘
    │            │             │
    └────────┬───┴─────────────┘
             │
...
Это делает систему эффективной: экономим память на тегах, но сохраняем уникальность каждой заметки!
